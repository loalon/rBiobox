...
#' x <- plotInteractivePCA(toyData$expressionData, data.frame(conditions=toyData$conditions, time=toyData$time), c("time", "conditions"))
#'
#' direct plot with custom color
#' plotInteractivePCA(toyData$expressionData, data.frame(conditions=toyData$conditions, time=toyData$time), c("conditions"), title="Treatment", colors = c("#F8766D", "#00BFC4", "#56B4E9"))
#'
#' reverse PC1
#' plotInteractivePCA(toyData$expressionData, data.frame(conditions=toyData$conditions, time=toyData$time), c("time", "conditions"), title="Treatment-week interaction", inverse = c(T,F,F))
#'
#' 3D version
#' plotInteractivePCA(toyData$expressionData, data.frame(conditions=toyData$conditions, time=toyData$time), c("time", "conditions"), title="Treatment-week interaction", inverse = c(T,F,F), model="3D")
#' @export
plotInteractivePCA <- function(data, meta, group, model= c("2D", "3D"), title="", colors = c(), inverse=c(FALSE,FALSE,FALSE)) {
  ...
  # Create plot object
  p <- if(model=="2D"){
    plotly::plot_ly(df3D, x = ~PC1, y = ~PC2,
    ...
    plotly::plot_ly(df3D, x = ~PC1, y = ~PC3, z = ~PC2,
            color = df3D[[groupNames[1]]],
            colors=colors,
            symbol = df3D[[groupNames[2]]],
            symbols= c("circle","square","cross","x"),
